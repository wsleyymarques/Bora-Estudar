...
export function SubjectSelector({ planId, onFinish, showFinishButton = false }: SubjectSelectorProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-32">
      <div className="grid lg:grid-cols-[1fr_350px] xl:grid-cols-[1fr_400px] gap-8 items-start">
        {/* LEFT COLUMN: CATALOG */}
        <div className="flex flex-col gap-6">
          {/* HEADER & SEARCH */}
          <div className="space-y-4 sticky top-[210px] z-30 bg-background pb-6 pt-2 -mt-2 -mx-2 px-2 border-b border-border/50 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.1)]">
            {/* Fill the gap above the search bar to prevent content from bleeding through between the header and search bar */}
            ...
            {!showNewForm ? (
              <Button
                ...
                className="w-full h-12 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 border-2 border-transparent font-black text-sm transition-all"
              >
        ...
        {/* RIGHT COLUMN: LINKED SUBJECTS */}
        <div className="lg:sticky lg:top-[210px] z-20">
          <div className="glass-card p-5 space-y-4 border-primary/20 bg-primary/5">
            <div className="flex items-center justify-between">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-primary">Matérias Adicionadas ({planSubjects.length})</h3>
            </div>
            ...
            {planSubjects.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {planSubjects.map(subject => (
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-background border border-border/50 text-foreground text-[11px] font-bold hover:bg-destructive/10 hover:border-destructive/20 hover:text-destructive transition-all group"
                  >
              ...
              <div className="text-center py-6">
                <p className="text-xs text-muted-foreground">Nenhuma matéria selecionada.</p>
                <p className="text-[10px] text-muted-foreground mt-1">Busque e adicione matérias ao lado.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed bottom-24 md:bottom-8 left-0 md:left-[var(--sidebar-width,16rem)] right-0 z-50 flex justify-center pointer-events-none">
          <div className="w-full max-w-xl px-4 lg:px-0 pointer-events-auto">
            <Button
              ...
              className="w-full h-14 rounded-[1.25rem] bg-primary text-primary-foreground font-black text-sm uppercase tracking-widest shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-0.5 transition-all active:scale-[0.98]"
            >
