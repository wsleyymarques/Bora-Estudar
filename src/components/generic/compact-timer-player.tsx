...
export function CompactTimerPlayer({
  ...
  return (
    <div
      className={cn(
        isFloating
          ? 'fixed left-3 right-3 bottom-[calc(5.5rem+env(safe-area-inset-bottom))] sm:left-auto sm:right-4 sm:bottom-4 z-40 sm:w-[340px]'
          : 'w-full',
        className,
      )}
    >
      <div
        className={cn(
          'workspace-panel p-2.5 md:p-3 flex flex-col gap-2 shadow-[0_12px_34px_hsl(var(--foreground)/0.18)]',
