...
import { ActiveScheduleSwitcher } from '@/components/generic/active-schedule-switcher';
          ...
          <div className="flex h-svh min-h-screen min-w-0 flex-1 flex-col">
            <header className="h-16 flex items-center gap-3 border-b border-border/60 px-3 md:px-5 bg-background/55 backdrop-blur sticky top-0 z-10">
              ...
              <div className="workspace-panel hidden sm:flex items-center px-4 py-2 text-sm text-muted-foreground w-full max-w-lg">
                Pesquisar materias, sessoes, notas...
              </div>
              <div className="hidden lg:flex">
                <ActiveScheduleSwitcher />
              </div>
            </header>
            <main
              className={cn(
                ...
                isSchedulePage
                  ? 'p-3 pb-[calc(6.75rem+env(safe-area-inset-bottom))] md:pb-3'
                  : 'p-3 md:p-5 lg:p-6 pb-[calc(6.75rem+env(safe-area-inset-bottom))] md:pb-5 lg:pb-6',
              )}
            >
              {children}
            </main>
